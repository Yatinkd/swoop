            ...
            Container(
              ...
              child: const Row(
                ...
                children: [
                  ...
                  SizedBox(width: 8),
                  Text('Event ended · Chat is read-only', style: TextStyle(color: Colors.orange, fontWeight: FontWeight.w600, fontSize: 13)),
                ],
              ),
            ),
          Expanded(
            child: StreamBuilder<List<Map<String, dynamic>>>(
              stream: supabase.from('messages').stream(primaryKey: ['id']).eq('plan_id', widget.plan['id']).order('created_at', ascending: false),
              builder: (context, snapshot) {
                if (!snapshot.hasData) return const Center(child: CircularProgressIndicator(color: AppColors.accent));
                final messages = snapshot.data!;
                if (messages.isEmpty) return Center(child: Text('No messages yet. Say hi!', style: TextStyle(color: AppColors.subtle)));
                          ...
                          children: [
                            if (!isMe)
                              Padding(
                                ...
                                child: Text(msg['sender_name'] ?? 'User', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12, color: AppColors.accent)),
                              ),
                            Text(msg['text'] ?? '', style: TextStyle(color: isMe ? Colors.white : AppColors.primary, fontSize: 15, height: 1.4)),
                            const SizedBox(height: 4),
                            Text(
                              timeago.format(DateTime.parse(msg['created_at'])),
                              style: TextStyle(fontSize: 10, color: isMe ? Colors.white54 : AppColors.subtle),
                            ),
                          ],
                        ),
                      ),
